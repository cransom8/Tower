...
test("human hero presentations resolve directly to TT hero keys", () => {
  const heroExpectations = [
    ["hero_king", "tt_king", "King"],
    ["hero_paladin", "tt_paladin", "Paladin"],
    ["hero_bishop", "tt_commander", "Bishop"],
  ];

  for (const [archetypeKey, expectedCatalogKey, expectedDisplayName] of heroExpectations) {
    ...
    assert.equal(presentation.displayName, expectedDisplayName);
