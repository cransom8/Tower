...
//      pre-built *_Controller.controller. Authored/imported materials are kept
//      wherever the asset pack provides them; empty slots fall back to the
//      generated shared unit material.
//   3. Registers the prefab in Assets/Registry/UnitPrefabRegistry.asset.
//
// Safe to re-run: overwrites existing prefabs and updates registry entries in
// place if the key already has a row.
//
...
public static class CreateHFCUnitPrefabs
{
    ...
    public static void Run()
    {
        ...
        foreach (var d in Defs)
        {
            ...
            var sourcePrefabPath = FindSourcePrefabPath(d);
            var sourcePrefab = string.IsNullOrEmpty(sourcePrefabPath)
                ? null
            ...
            // Prefer the authored pack prefab so weapon meshes/material slots survive.
            var go = Object.Instantiate(sourcePrefab != null ? sourcePrefab : fbxAsset);
            ...
            // Keep authored/imported materials where present and only fill
            // empty slots with the shared unit material.
            foreach (var smr in go.GetComponentsInChildren<SkinnedMeshRenderer>(true))
            {
                smr.sharedMaterials = RepairMissingMaterials(smr.sharedMaterials, mat);
            }

            foreach (var mr in go.GetComponentsInChildren<MeshRenderer>(true))
            {
                mr.sharedMaterials = RepairMissingMaterials(mr.sharedMaterials, mat);
            }
        ...
        Debug.Log($"[CreateHFCUnitPrefabs] Done — rebuilt: {rebuilt}  failed: {failed}");
    }
    ...
    static string FindSourcePrefabPath(Def d)
    {
        var lastSlash = d.fbx.LastIndexOf("/FBX", System.StringComparison.OrdinalIgnoreCase);
        if (lastSlash <= 0) return null;

        var unitRoot = d.fbx.Substring(0, lastSlash);
        var prefabDir = unitRoot + "/Prefabs";
        if (!AssetDatabase.IsValidFolder(prefabDir)) return null;
        ...
        foreach (var guid in guids)
        {
            ...
            if (firstPrefab == null) firstPrefab = path;

            var fileName = Path.GetFileNameWithoutExtension(path).ToLowerInvariant();
            if (fileName.Contains("pbr"))
                return path;
        }
    ...
    static Material[] RepairMissingMaterials(Material[] existing, Material fallback)
    {
        if (existing == null || existing.Length == 0)
            return new[] { fallback };

        var repaired = new Material[existing.Length];
        for (int i = 0; i < existing.Length; i++)
            repaired[i] = existing[i] != null ? existing[i] : fallback;
