...
namespace CastleDefender.Game
{
    ...
    {
        public const float UnitBillboardScaleFactor = 0.26f;
        ...
        static readonly Vector3 UnitBarLocalScale = new(1.30f, 2.20f, 1f);
        ...
        public static void ConfigureUnitBar(Transform barRoot)
        {
            ...
            if (billboard != null)
            {
                ...
                billboard.CompensatePerspectiveZoom = true;
                billboard.CompensateZoomIn = true;
            }
        }

        public static void ConfigureBuildingHud(Transform hudRoot)
        {
            if (hudRoot == null)
                return;
        ...
        static void CreateFillPiece(Transform parent, string name, Vector3 localPosition, Vector3 localScale, bool rounded)
        {
            ...
            primitive.transform.localScale = rounded
                ? new Vector3(localScale.x * 0.75f, localScale.y, localScale.z)
                : localScale;
        ...
        {
            // Keep frame generation free of implicit SphereCollider creation.
            var primitive = GameObject.CreatePrimitive(PrimitiveType.Cube);
            primitive.name = name;
            primitive.transform.SetParent(parent, false);
            primitive.transform.localPosition = localPosition;
