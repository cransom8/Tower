...
namespace CastleDefender.UI
{
    public class LobbyUI : MonoBehaviour
    {
        ...
        IEnumerator WarmLobbyT1Content()
        {
            ...
            yield return remoteContent.PreloadCriticalContentForSession(requester: "LobbyUI.WarmLobbyT1Content");

            if (!remoteContent.HasCompletedCriticalPreload)
            {
                Debug.LogWarning($"[LobbyUI] T1 warmup did not complete in lobby: {remoteContent.LastError}");
                if (!_inQueue && _currentLobby == null)
                    SetStatus("Remote gameplay content failed to warm up. It will retry before loadout.");
            }
