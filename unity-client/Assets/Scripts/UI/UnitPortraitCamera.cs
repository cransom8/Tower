...
namespace CastleDefender.UI
{
    public class UnitPortraitCamera : MonoBehaviour
    {
        ...
        public void ShowUnit(string key)
        {
            ...
            {
                Debug.LogWarning($"[UnitPortraitCamera] Prefab '{prefab.name}' for '{key}' has no renderers.");
                return;
            }
            ...
            foreach (var skinned in _staged.GetComponentsInChildren<SkinnedMeshRenderer>(true))
            {
                if (skinned.sharedMesh == null) missingSkinnedMeshes++;
                var mats = skinned.sharedMaterials;
                if (mats == null || mats.Length == 0) missingMaterials++;
                else
                {
                    for (int i = 0; i < mats.Length; i++)
                        if (mats[i] == null) missingMaterials++;
                }
            }
            foreach (var filter in _staged.GetComponentsInChildren<MeshFilter>(true))
            {
                if (filter.sharedMesh == null) missingStaticMeshes++;
            }
            ...
            Debug.Log($"[UnitPortraitCamera] Prepared portrait for '{key}' using prefab '{prefab.name}' with {renderers.Length} renderers. missingSkinnedMeshes={missingSkinnedMeshes} missingStaticMeshes={missingStaticMeshes} missingMaterials={missingMaterials} missingController={missingController}");
        }
        ...
        void FrameStagedUnit(Bounds bounds)
        {
            ...
            Cam.transform.position = focusPoint + new Vector3(0f, 0.05f, -camDistance);
            Cam.transform.rotation = Quaternion.identity;
            Cam.transform.LookAt(focusPoint + new Vector3(0f, height * 0.06f, 0f));
